...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        "上傳 Excel 總表（.xlsx）",
        type=["xlsx"],
        help="欄位：學校名稱、學年度學期、學科、老師姓名、任教班級、任教年段、教科書版本、特殊身分備註、班級人數",
    )
    st.markdown("""
    <div class="upload-hint">
    📋 <b>必要欄位（固定 10 欄）：</b><br>
    學校名稱、學年度學期、學科、老師姓名、任教班級、任教年段、教科書版本、特殊身分備註、班級人數
    </div>
    """, unsafe_allow_html=True)
...
with tab5:
    ...
    # 顯示用 DataFrame：依 Excel 欄位順序排列（共 9 欄，不含學校名稱與學年度學期）
